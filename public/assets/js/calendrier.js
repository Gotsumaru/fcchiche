(function () {
  ...
  async function loadCalendarForTeam(teamId) {
    ...
    try {
      const response = await state.api.getMatchsByEquipe(teamId, { isResult: false, limit: MAX_MATCHES });
      ...
      if (matchs.length === 0) {
        setListMessage('Aucun match programmé pour cette équipe pour le moment.');
        return;
      }
