import { useState, useEffect, useRef } from 'react'
import apiClient from '../api'
import MatchCard from '../components/MatchCard'
import ResultCard from '../components/ResultCard'
import { useHorizontalScroll } from '../hooks/useHorizontalScroll'

// Helper function to format date
function formatDate(dateString) {
  if (!dateString) return 'TBA'
  try {
    const date = new Date(dateString)
    const day = date.toLocaleDateString('fr-FR', { weekday: 'short', day: '2-digit' })
    const time = date.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })
    return `${day} ${time}`
  } catch (e) {
    return dateString
  }
}

export default function HomePage() {
  const [upcomingMatches, setUpcomingMatches] = useState([])
  const [latestResults, setLatestResults] = useState([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState(null)
  const eventsScrollerRef = useRef(null)
  const resultsScrollerRef = useRef(null)

  // Initialize horizontal scroll for events and results
  useHorizontalScroll(eventsScrollerRef)
  useHorizontalScroll(resultsScrollerRef)

  useEffect(() => {
    loadData()
  }, [])

  async function loadData() {
    try {
      setError(null)
      const upcoming = await apiClient.getUpcomingMatches(6)
      setUpcomingMatches(upcoming)

      const results = await apiClient.getLatestResults(6)
      setLatestResults(results)
    } catch (error) {
      console.error('Failed to load data:', error)
      setError('Impossible de charger les données. Vérifiez que le serveur PHP est actif.')
    } finally {
      setLoading(false)
    }
  }

  return (
    <>
      {/* HERO SECTION */}
      <section className="hero" data-reveal="hero" data-reveal-delay="0">
        <div className="container hero__container">
          <header className="hero__intro" data-reveal="hero" data-reveal-delay="0">
            <h1 className="hero__title" id="hero-title">F.C. Chiché</h1>
            <span className="hero__tagline" role="text">
              <img
                className="hero__tagline-logo"
                src="/assets/images/logo.svg"
                width="40"
                height="40"
                alt=""
                aria-hidden="true"
                loading="lazy"
                decoding="async"
              />
              <span className="hero__tagline-text">Pour l'amour du maillot vert</span>
            </span>
          </header>
          <figure className="hero__visual" data-reveal="hero" data-reveal-delay="0.15">
            <div className="hero__media">
              <picture>
                <source
                  srcSet="/assets/images/home-low.webp"
                  type="image/webp"
                  media="(max-width: 30rem)"
                />
                <source
                  srcSet="
                    /assets/images/home-480.webp 480w,
                    /assets/images/home-800.webp 800w,
                    /assets/images/home-1200.webp 1200w
                  "
                  type="image/webp"
                  sizes="(min-width: 70rem) 960px, (min-width: 48rem) 88vw, 100vw"
                />
                <source
                  srcSet="/assets/images/home.jpg 2048w"
                  type="image/jpeg"
                  sizes="(min-width: 70rem) 960px, (min-width: 48rem) 88vw, 100vw"
                />
                <img
                  src="/assets/images/home.jpg"
                  width="2048"
                  height="1152"
                  alt="Les joueurs du FC Chiché rentrent sur la pelouse du stade municipal"
                  loading="eager"
                  fetchPriority="high"
                  decoding="async"
                />
              </picture>
            </div>
          </figure>
          <div className="hero__content" data-reveal="hero" data-reveal-delay="0.3">
            <p className="hero__subtitle">
              Ne manquez rien de la vie du club : résultats, matchs, événements et moments forts !
            </p>
            <div className="hero__actions" role="group" aria-label="Actions principales">
              <a className="btn btn--primary" href="/matchs">Voir le calendrier</a>
              <a className="btn btn--primary" href="/resultats">Derniers résultats</a>
            </div>
          </div>
        </div>
      </section>

      {/* CLUB STORY SECTION */}
      <section className="section section--story" aria-labelledby="story-title">
        <div className="container">
          <div className="story__intro" data-reveal data-reveal-delay="0">
            <span className="section__eyebrow">Le club</span>
            <h2 className="section__title" id="story-title">Un écrin pour les verts et blancs</h2>
            <p className="section__subtitle">
              Des vestiaires à la buvette, tout est pensé pour offrir le meilleur à <span style={{fontWeight: 'bold', color: '#000000'}}>nos licenciés et nos supporters</span>. Entrez dans les coulisses <span style={{fontWeight: 'bold', color: '#000000'}}>d'un club familial</span> où passion et performance vont de pair.
            </p>
          </div>
          <div className="story__layout" data-reveal data-reveal-delay="0.1">
            <div className="club-showcase" role="list">
              <article className="club-showcase__item" role="listitem" data-reveal data-reveal-delay="0.12">
                <figure className="club-showcase__media">
                  <div className="club-showcase__media-frame">
                    <picture>
                      <source
                        srcSet="
                          /assets/images/fcchiche_tableau.jpg 480w,
                          /assets/images/fcchiche_tableau.jpg 800w,
                          /assets/images/fcchiche_tableau.jpg 1200w
                        "
                        type="image/jpeg"
                        sizes="(min-width: 64rem) 480px, (min-width: 48rem) 60vw, 90vw"
                      />
                      <source
                        srcSet="/assets/images/fcchiche_tableau.jpg 1200w"
                        type="image/png"
                        sizes="(min-width: 64rem) 480px, (min-width: 48rem) 60vw, 90vw"
                      />
                      <img
                        src="/assets/images/fcchiche_tableau.jpg"
                        width="1300"
                        height="866"
                        alt="Les nouveaux vestiaires du FC Chiché préparés avant un match"
                        loading="lazy"
                        decoding="async"
                      />
                    </picture>
                  </div>
                </figure>
                <div className="club-showcase__content">
                  <h3 className="club-showcase__title">Tableau Classements</h3>
                  <p className="club-showcase__text">
                    Nouveau tableau de classement sponsorisé par la <strong>Carrière de Luche</strong>, merci à eux !
                  </p>
                  <p className="club-showcase__text">
                    Il nous permet d'afficher <strong>les rencontres et les classements</strong> pour nos trois équipes séniors.
                  </p>
                </div>
              </article>
              <article className="club-showcase__item" role="listitem" data-reveal data-reveal-delay="0.24">
                <figure className="club-showcase__media">
                  <div className="club-showcase__media-frame">
                    <picture>
                      <source
                        srcSet="/assets/images/bal_du_foot.png 1200w"
                        type="image/PNG"
                        sizes="(min-width: 64rem) 480px, (min-width: 48rem) 60vw, 90vw"
                      />
                      <source
                        srcSet="/assets/images/bal_du_foot.png 1200w"
                        type="image/PNG"
                        sizes="(min-width: 64rem) 480px, (min-width: 48rem) 60vw, 90vw"
                      />
                      <img
                        src="/assets/images/bal_du_foot.png"
                        width="1600"
                        height="1067"
                        alt="Séance d'entraînement des jeunes joueurs du FC Chiché"
                        loading="lazy"
                        decoding="async"
                      />
                    </picture>
                  </div>
                </figure>
                <div className="club-showcase__content">
                  <h3 className="club-showcase__title">Bal du Foot</h3>
                  <p className="club-showcase__text">
                    Le Football Club de Chiche organise un dinner dansant, avec traiteur, serveurs, un orchestre et de la bonne ambiance.
                  </p>
                  <p className="club-showcase__text">
                    Il a lieu tous les ans le 10 Novembre pour accueil <strong>300+ personnes</strong>.
                  </p>
                </div>
              </article>
              <article className="club-showcase__item" role="listitem" data-reveal data-reveal-delay="0.36">
                <figure className="club-showcase__media">
                  <div className="club-showcase__media-frame">
                    <picture>
                      <source srcSet="/assets/images/Ecole_de_foot.jpg" type="image/webp" />
                      <source srcSet="/assets/images/Ecole_de_foot.jpg" type="image/jpeg" />
                      <img
                        src="/assets/images/Ecole_de_foot.jpg"
                        width="1792"
                        height="1024"
                        alt="Bénévoles du FC Chiché en service derrière la buvette"
                        loading="lazy"
                        decoding="async"
                      />
                    </picture>
                  </div>
                </figure>
                <div className="club-showcase__content">
                  <h3 className="club-showcase__title">Ecole de Foot</h3>
                  <p className="club-showcase__text">
                    Recrutement d'un alternant pour la gestion de nos effectifs jeunes.
                  </p>
                  <p className="club-showcase__text">
                    Avec <strong>50+ licenciés jeunes</strong> et une équipe évoluant dans chaque catégorie allant de <strong>U6 à U17</strong>.
                  </p>
                </div>
              </article>
            </div>
          </div>
        </div>
      </section>

      {/* UPCOMING MATCHES SECTION */}
      <section className="section section--alt section--events" aria-labelledby="events-title">
        <div className="container_events">
          <div className="section__header" data-reveal data-reveal-delay="0">
            <h2 className="section__title section__title-events" id="events-title">Les prochains matchs.</h2>
            <a className="section__action" href="/matchs">Voir le calendrier</a>
          </div>
          <div
            ref={eventsScrollerRef}
            className="home-scroll home-scroll--events"
            data-component="home-events"
            data-reveal
            data-reveal-delay="0.12"
          >
            {error && (
              <div style={{ padding: '20px', color: '#d32f2f', textAlign: 'center' }}>
                {error}
              </div>
            )}
            {loading && (
              <div style={{ padding: '20px', textAlign: 'center' }}>
                Chargement des matchs...
              </div>
            )}
            {!loading && upcomingMatches.length === 0 && (
              <div style={{ padding: '20px', textAlign: 'center' }}>
                Aucun match à venir
              </div>
            )}
            <div className="home-scroll__track home-scroll__track-events" data-component="home-events-list" aria-live="polite">
              {!loading && upcomingMatches.map((match) => (
                <MatchCard key={match.id} match={match} />
              ))}
            </div>
            <div className="home-scroll__controls">
              <button
                type="button"
                className="home-scroll__control"
                data-action="scroll-prev"
                aria-label="Voir les matchs précédents"
              ></button>
              <button
                type="button"
                className="home-scroll__control"
                data-action="scroll-next"
                aria-label="Voir les prochains matchs"
              ></button>
            </div>
          </div>
        </div>
      </section>

      {/* LATEST RESULTS SECTION */}
      <section className="section section--alt section--events" aria-labelledby="results-title">
        <div className="container_events">
          <div className="section__header" data-reveal data-reveal-delay="0">
            <h2 className="section__title section__title-events" id="results-title">Derniers résultats du club.</h2>
            <a className="section__action" href="/resultats">Voir les résultats</a>
          </div>
          <div
            ref={resultsScrollerRef}
            className="home-scroll home-scroll--events"
            data-component="home-results"
            data-reveal
            data-reveal-delay="0.12"
          >
            {error && (
              <div style={{ padding: '20px', color: '#d32f2f', textAlign: 'center' }}>
                {error}
              </div>
            )}
            {loading && (
              <div style={{ padding: '20px', textAlign: 'center' }}>
                Chargement des résultats...
              </div>
            )}
            {!loading && latestResults.length === 0 && (
              <div style={{ padding: '20px', textAlign: 'center' }}>
                Aucun résultat
              </div>
            )}
            <div className="home-scroll__track home-scroll__track-events" data-component="home-results-list" aria-live="polite">
              {!loading && latestResults.map((result) => (
                <ResultCard key={result.id} result={result} />
              ))}
            </div>
            <div className="home-scroll__controls">
              <button
                type="button"
                className="home-scroll__control"
                data-action="scroll-prev"
                aria-label="Voir les résultats précédents"
              ></button>
              <button
                type="button"
                className="home-scroll__control"
                data-action="scroll-next"
                aria-label="Voir les résultats suivants"
              ></button>
            </div>
          </div>
        </div>
      </section>

      {/* LOCATION SECTION */}
      <section className="section" aria-labelledby="location-title">
        <div className="container">
          <div className="section__header" data-reveal data-reveal-delay="0">
            <span className="section__eyebrow">Notre stade</span>
            <h2 className="section__title" id="location-title">Retrouvez-nous à Chiché</h2>
            <p className="section__subtitle">
              Venez encourager les verts et blancs au complexe sportif. Buvette, tribunes couvertes et grand parking sont à votre disposition.
            </p>
          </div>
          <div className="location" data-reveal data-reveal-delay="0.12">
            <div className="location__map">
              <iframe
                title="Localisation du FC Chiché"
                src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d2706.655193924687!2d-0.543!3d46.783!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x480799b0517d8623%3A0x5d6587b9e90b8e0!2sChich%C3%A9!5e0!3m2!1sfr!2sfr!4v1700000000000"
                loading="lazy"
                referrerPolicy="no-referrer-when-downgrade"
                allowFullScreen=""
              ></iframe>
            </div>
            <div className="location__details">
              <h3>Complexe sportif de la Brelandière</h3>
              <address>
                Rue du Stade<br />
                79350 Chiché, France
              </address>
              <p className="location__schedule">
                Accueil du secrétariat&nbsp;: lundi au vendredi 18h-20h • Buvette ouverte les jours de match dès 1h avant le coup d'envoi.
              </p>
              <a className="btn btn--primary" href="https://maps.google.com/?daddr=Chich%C3%A9" target="_blank" rel="noopener noreferrer">
                Comment venir
              </a>
            </div>
          </div>
        </div>
      </section>

      {/* PARTNERS SECTION */}
      <section className="section section--alt" aria-labelledby="partners-title">
        <div className="container">
          <div className="section__header" data-reveal data-reveal-delay="0">
            <span className="section__eyebrow">Soutiens</span>
            <h2 className="section__title" id="partners-title">Nos partenaires</h2>
            <p className="section__subtitle">Un grand merci aux entreprises locales qui accompagnent le FC Chiché.</p>
          </div>
          <div className="partner-carousel" data-component="partner-carousel" data-reveal data-reveal-delay="0.12">
            <button
              type="button"
              className="partner-carousel__control partner-carousel__control--prev"
              data-action="scroll-prev"
              aria-label="Voir le partenaire précédent"
            ></button>
            <div className="partner-carousel__viewport" role="list">
              <a className="partner-card" role="listitem" href="https://example.com" target="_blank" rel="noopener noreferrer">
                <img
                  src="/assets/images/logo.svg"
                  width="160"
                  height="160"
                  alt="Logo de BCZ Construction"
                  loading="lazy"
                  decoding="async"
                  data-variant="emerald"
                />
                <span>BCZ Construction</span>
              </a>
              <a className="partner-card" role="listitem" href="https://example.com" target="_blank" rel="noopener noreferrer">
                <img
                  src="/assets/images/placeholder-logo.svg"
                  width="160"
                  height="160"
                  alt="Logo de Boche Chaussure"
                  loading="lazy"
                  decoding="async"
                  data-variant="slate"
                />
                <span>Boche Chaussure</span>
              </a>
              <a className="partner-card" role="listitem" href="https://example.com" target="_blank" rel="noopener noreferrer">
                <img
                  src="/assets/images/placeholder-logo.svg"
                  width="160"
                  height="160"
                  alt="Logo de Cholet Traiteur"
                  loading="lazy"
                  decoding="async"
                  data-variant="forest"
                />
                <span>Cholet Traiteur</span>
              </a>
              <a className="partner-card" role="listitem" href="https://example.com" target="_blank" rel="noopener noreferrer">
                <img
                  src="/assets/images/placeholder-logo.svg"
                  width="160"
                  height="160"
                  alt="Logo de Chiché Automobile"
                  loading="lazy"
                  decoding="async"
                  data-variant="charcoal"
                />
                <span>Chiché Automobile</span>
              </a>
              <a className="partner-card" role="listitem" href="https://example.com" target="_blank" rel="noopener noreferrer">
                <img
                  src="/assets/images/placeholder-logo.svg"
                  width="160"
                  height="160"
                  alt="Logo de Maison Dubois"
                  loading="lazy"
                  decoding="async"
                  data-variant="amber"
                />
                <span>Maison Dubois</span>
              </a>
              <a className="partner-card" role="listitem" href="https://example.com" target="_blank" rel="noopener noreferrer">
                <img
                  src="/assets/images/placeholder-logo.svg"
                  width="160"
                  height="160"
                  alt="Logo de Crédit Bocage"
                  loading="lazy"
                  decoding="async"
                  data-variant="silver"
                />
                <span>Crédit Bocage</span>
              </a>
            </div>
            <button
              type="button"
              className="partner-carousel__control partner-carousel__control--next"
              data-action="scroll-next"
              aria-label="Voir le partenaire suivant"
            ></button>
          </div>
        </div>
      </section>
    </>
  )
}
